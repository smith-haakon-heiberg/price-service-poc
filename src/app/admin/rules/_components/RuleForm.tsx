...
import type { PriceRule, RuleCondition, PriceAdjustment, QuantityBreak } from "@/domain/types";
...
function quantityBreakToRow(qb: QuantityBreak): QuantityBreakRow {
  return {
    minQuantity: String(qb.minQuantity),
    maxQuantity: qb.maxQuantity !== undefined ? String(qb.maxQuantity) : "",
    adjustmentType: qb.adjustment.type,
    adjustmentValue: String(qb.adjustment.value),
  };
}

interface RuleFormProps {
  mode: "create" | "edit";
  initialRule?: PriceRule;
}

export default function RuleForm({ mode, initialRule }: RuleFormProps) {
  const router = useRouter();

  const [name, setName] = useState(initialRule?.name ?? "");
  const [type, setType] = useState<string>(initialRule?.type ?? RuleType.BASE_CATEGORY);
  const [priority, setPriority] = useState<number>(initialRule?.priority ?? DEFAULT_PRIORITY.BASE_CATEGORY);
  const [enabled, setEnabled] = useState(initialRule?.enabled ?? true);
  const [override, setOverride] = useState(initialRule?.override ?? false);
  const [validFrom, setValidFrom] = useState(initialRule?.validFrom ?? "");
  const [validTo, setValidTo] = useState(initialRule?.validTo ?? "");
  const [pricelistId, setPricelistId] = useState(initialRule?.pricelistId ?? "");

  const [adjustmentType, setAdjustmentType] = useState<string>(
    initialRule?.adjustment.type ?? AdjustmentType.PERCENTAGE_MARKUP
  );
  const [adjustmentValue, setAdjustmentValue] = useState(
    initialRule ? String(initialRule.adjustment.value) : ""
  );

  const [conditions, setConditions] = useState<ConditionRow[]>(
    initialRule?.conditions.map(conditionToRow) ?? []
  );
  const [quantityBreaks, setQuantityBreaks] = useState<QuantityBreakRow[]>(
    initialRule?.quantityBreaks?.map(quantityBreakToRow) ?? []
  );
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      const url =
        mode === "edit"
          ? `/api/prices/rules/${initialRule!.id}`
          : "/api/prices/rules";
      ...
      router.push(`/admin/rules/${saved.id}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : `Failed to ${mode} rule`);
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6 max-w-3xl">
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          type="button"
          onClick={() => router.push(mode === "edit" ? `/admin/rules/${initialRule!.id}` : "/admin/rules")}
