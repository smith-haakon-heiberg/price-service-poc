...
export async function GET(_request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    const result = await service.getPricelistWithRules(id);
    if (!result) {
      return errorResponse(`Pricelist not found: ${id}`, 'NOT_FOUND', 404);
    }
    return jsonResponse(result);
  } catch (err) {
    console.error(`Unexpected error in GET /api/prices/pricelists/${id}:`, err);
...
export async function PUT(request: NextRequest, { params }: RouteContext) {
  ...
  try {
    const service = getPriceService();
    const updated = await service.updatePricelist(id, updates);
    ...
    console.error(`Unexpected error in PUT /api/prices/pricelists/${id}:`, err);
    return errorResponse('Internal server error', 'INTERNAL_ERROR', 500);
  }
}
...
export async function DELETE(_request: NextRequest, { params }: RouteContext) {
  ...
  try {
    ...
    console.error(`Unexpected error in DELETE /api/prices/pricelists/${id}:`, err);
    return errorResponse('Internal server error', 'INTERNAL_ERROR', 500);
  }
}
